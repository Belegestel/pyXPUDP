...
class _CallbackHandle():
    ...
    def remove(self):
        if self._is_valid():
            self._dispatcher._remove_callback(self._key, self._id)
        else:
...
class _CallbackDispatcher():
    ...
    def _add_callback(self, callback, key=None):
        ...
        self._callbacks[key][callback_id] = callback
    ...
    def _run_callbacks(self, key, value):
        funcs_to_run = list(self._callbacks[None].values())
        funcs_to_run.extend(self._callbacks.get(key, {}).items())
